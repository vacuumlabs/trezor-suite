...
import { CoinjoinClient, coordinator, middleware } from '@trezor/coinjoin';
...
export const getOwnershipProof = (event: any) => (dispatch: Dispatch, getState: GetState) => {
    console.warn('--getOwnership', event);
    // @ts-ignore
...
export const registerInput =
    (utxo: AccountUtxo) => async (dispatch: Dispatch, getState: GetState) => {
        ...
        CoinjoinClient.addInput({ vin: utxo, addresses: account.addresses!.change });
        ...
        const realVsizeCredentials = await middleware.getRealCredentials(
            [round.maxVsizeAllocationPerAlice - inputSize],
            vsizeCredentials,
            round.vsizeCredentialIssuerParameters,
