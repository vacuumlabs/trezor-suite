...
export const onStatusUpdate =
    (status: Round[] | null) => (dispatch: Dispatch, getState: GetState) => {
        ...
        // Find changed round
        const changedRounds = status.flatMap(
            r => coinjoin.status.find(rr => rr.id === r.id && rr.phase !== r.phase) ?? [],
        );
        console.warn('Changed!', changedRounds);
...
export const registerInput =
    (utxo: AccountUtxo) => async (dispatch: Dispatch, getState: GetState) => {
        ...
        const zeroAmountCredentials = await wabisabi.crypto('zero-amount-request', {
            credentialIssuerParameters: round.amountCredentialIssuerParameters,
        });

        const zeroVsizeCredentials = await wabisabi.crypto('zero-amount-request', {
        ...
        const amountCredentials = await wabisabi.crypto('handle-response', {
            credentialIssuerParameters: round.amountCredentialIssuerParameters,
        ...
        const vsizeCredentials = await wabisabi.crypto('handle-response', {
            credentialIssuerParameters: round.vsizeCredentialIssuerParameters,
        ...
        const realAmountCredentials = await wabisabi.crypto('create-request', {
            amountsToRequest: [amount],
            credentialIssuerParameters: round.amountCredentialIssuerParameters,
            maxCredentialValue: round.maxAmountCredentialValue,
            credentialsToPresent: amountCredentials,
        });
        const realVsizeCredentials = await wabisabi.crypto('create-request', {
            amountsToRequest: [round.maxVsizeAllocationPerAlice - inputSize],
            credentialIssuerParameters: round.vsizeCredentialIssuerParameters,
            maxCredentialValue: round.maxVsizeCredentialValue,
            credentialsToPresent: vsizeCredentials,
        });
...
export const connectionConfirmation =
    (alice: Alice) => async (dispatch: Dispatch, getState: GetState) => {
        try {
            const zeroAmountCredentials = await wabisabi.crypto('zero-amount-request', {
            ...
            const confirmationData = await wabisabi.post('connection-confirmation', {
                ...
                zeroAmountCredentialRequests: zeroAmountCredentials.credentialsRequest,
                realAmountCredentialRequests: alice.realAmountCredentials.credentialsRequest,
                zeroVsizeCredentialRequests: zeroVsizeCredentials.credentialsRequest,
                realVsizeCredentialRequests: alice.realVsizeCredentials.credentialsRequest,
            });
...
const decompose = async (amount: number, output: any, alice: Alice) => {
    const realAmountCredentialsToPresent = await wabisabi.crypto('handle-response', {
        credentialIssuerParameters: alice.round.amountCredentialIssuerParameters,
    ...
    const realVsizeCredentialsToPresent = await wabisabi.crypto('handle-response', {
        credentialIssuerParameters: alice.round.vsizeCredentialIssuerParameters,
    ...
    const zeroAmountCredentialsToPresent = await wabisabi.crypto('handle-response', {
        credentialIssuerParameters: alice.round.amountCredentialIssuerParameters,
    ...
    const zeroVsizeCredentialsToPresent = await wabisabi.crypto('handle-response', {
        credentialIssuerParameters: alice.round.vsizeCredentialIssuerParameters,
    ...
    const issuanceAmountCredentials = await wabisabi.crypto('create-request', {
        amountsToRequest: [amount, alice.amount - amount],
        credentialIssuerParameters: alice.round.amountCredentialIssuerParameters,
        maxCredentialValue: alice.round.maxAmountCredentialValue,
        credentialsToPresent: realAmountCredentialsToPresent,
    });
    const issuanceVsizeCredentials = await wabisabi.crypto('create-request', {
        amountsToRequest: [
        ...
        credentialsToPresent: realVsizeCredentialsToPresent,
    });
    ...
    return Promise.all(
        outputRealAmountCredentials.map(
            (c: any, i: any) =>
                // eslint-disable-next-line no-async-promise-executor
                new Promise(async resolve => {
                    const outputAmountCredentials = await wabisabi.crypto('create-request', {
                        amountsToRequest: [0],
                        credentialIssuerParameters: alice.round.amountCredentialIssuerParameters,
                        maxCredentialValue: alice.round.maxAmountCredentialValue,
                        credentialsToPresent: [
                            outputRealAmountCredentials[i],
                            zeroAmountCredentialsToPresent[i],
                        ],
                    });
                    const outputVsizeCredentials = await wabisabi.crypto('create-request', {
                        amountsToRequest: [outputRealVsizeCredentials[i].value - alice.outputSize],
                        credentialIssuerParameters: alice.round.vsizeCredentialIssuerParameters,
                        maxCredentialValue: alice.round.maxVsizeCredentialValue,
                        credentialsToPresent: [
                            outputRealVsizeCredentials[i],
                            zeroVsizeCredentialsToPresent[i],
                        ],
                    });
...
export const registerOutputs =
    (alice: Alice, PROOFS: any) => async (dispatch: Dispatch, getState: GetState) => {
        ...
        const registeredInputs = wabisabi.getEvents('InputAdded', round.coinjoinState.events);
        ...
        const outputAmounts: number[] = await wabisabi.crypto('decompose-amounts', {
            internalAmounts: [alice.amount],
            externalAmounts: registeredInputs
                .filter(i => i.coin.outpoint !== alice.outpoint)
                .map(a => a.coin.txOut.value),
        ...
        const outputz = alice.addresses.slice(outputAmounts.length).map(addr => {
            // OP_1: 51
            const scriptPubKey =
        ...
        const outputs = await Promise.all(
            outputAmounts.slice(1).map((amount, i) => decompose(amount, outputz[i], alice)),
        );
...
export const trezorPaymentRequest = (outputs: any, change_addresses: string[]) =>
